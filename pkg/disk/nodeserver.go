...
func (ns *nodeServer) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
    ...
    if pvc.Spec.DataSource != nil {
        ...
        if !needResize {
            if needResize, err = needResizeVolume(device, targetPath, mounter); err != nil {
                klog.Errorf("NodeStageVolume: could not determine if volume %s needs to be resized: %v", req.VolumeId, err)
                return &csi.NodeStageVolumeResponse{}, nil
            }
        }
        if needResize {
